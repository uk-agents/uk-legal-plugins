...
- `/product-legal-uk:matter-workspace new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `/product-legal-uk:matter-workspace list` — list matters with status and active flag
- `/product-legal-uk:matter-workspace switch <slug>` — set the active matter
- `/product-legal-uk:matter-workspace close <slug>` — archive a matter (move to `~/.claude/plugins/config/uk-legal-plugins/product-legal-uk/matters/_archived/`, never delete)
- `/product-legal-uk:matter-workspace none` — detach from any active matter, work at practice-level only

## Instructions

1. Read `~/.claude/plugins/config/uk-legal-plugins/product-legal-uk/CLAUDE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `/product-legal-uk:cold-start-interview --redo` and select a private-practice setting. Otherwise, you don't need `/matter-workspace` at all." Don't error — the disabled state is the expected one for in-house users.
2. Apply the storage layout and subcommand logic below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/uk-legal-plugins/product-legal-uk/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/uk-legal-plugins/product-legal-uk/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/uk-legal-plugins/product-legal-uk/matters/<slug>/` to `~/.claude/plugins/config/uk-legal-plugins/product-legal-uk/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.claude/plugins/config/uk-legal-plugins/product-legal-uk/
...
Slugs are lowercase with hyphens. Examples: `acme-launch-2026`, `zenith-claims-review`, `vendor-xyz-nda`.
...
2. Run the intake interview:
   ...
   - **Matter type** (read the plugin's practice profile for typical categories; for product-legal-uk: launch | feature review | marketing claims review | risk deep dive | product area (standing) | regulatory inquiry | other)
   - **UK jurisdiction(s)** (England & Wales | Scotland | Northern Ireland | UK-wide | cross-border UK/EEA)
   ...
   - **Matter-specific overrides to the practice playbook** (e.g., "client is FCA-authorised — financial promotions approver is in-house", "counterparty is a strategic partner — relationship-preserving tone", "Scottish law governs — check Scots law differences")
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`/product-legal-uk:matter-workspace switch <slug>`)"

### `list`

Enumerate `matters/*/matter.md`. Read each file's front-matter or first few lines to extract status. Print a table:

| Slug | Client | Matter type | Jurisdiction | Status | Opened | Active |
|---|---|---|---|---|---|---|
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `/product-legal-uk:matter-workspace new <slug>`.
...
**Opened:** [YYYY-MM-DD]
**Status:** active
**Jurisdiction(s):** [England & Wales | Scotland | Northern Ireland | UK-wide | cross-border UK/EEA]
...
[launch | feature review | marketing claims review | risk deep dive | product area (standing) | regulatory inquiry | other — with one-line rationale]
...
- [e.g., "Financial promotions approver: [name] — FCA-authorised, confirmed for this matter."]
- [e.g., "Governing law: Scots law — flag differences from England & Wales analysis."]
...
Intake completed. Slug: `[slug]`. Jurisdiction(s): [list]. Status: active.
[Any initial context worth preserving beyond matter.md — e.g., "Opened in response to CMA inquiry into [feature]."]
```
...
When it's `on`, a skill may read files across matter folders only when the user explicitly asks it to (e.g., "compare our position on financial promotions approvals across the last three client matters"). Even when `on`, the default is to load only the active matter unless the user asks for a cross-matter view.
