...
4. **Chart(s).** One or two max. Pick the one that shows the shape:
   ...
   - **Category breakdown** (pie or stacked bar): counts by type. Use for OSS licences, contract types, matter categories.
...
- **Claude Code:** Write the same HTML file to the plugin's outputs folder (`~/.claude/plugins/config/uk-legal-plugins/<plugin>/outputs/dashboard-<topic>-<date>.html`) and tell the user to open it. Also produce a markdown version with Unicode block charts for the summary stats so the user can see the shape without leaving the terminal.
- **Excel (optional, where it fits):** For `renewal-tracker` and anything the user will take into a meeting or share with a non-technical stakeholder. Apply the formula-injection defence.
- **Escape untrusted input (apply every dashboard, every time).** Every value that came from outside this session — counterparty contract text, diligence findings, vendor names, matter descriptions, any user- or document-supplied string — must be HTML-escaped before it lands in the document. Escape `&`, `<`, `>`, `"`, `'` into entities when writing into table cells, summary lines, chart labels, and tooltip text. In the inline JS sorter/filter, set cell text via `textContent`, never `innerHTML`. Do not emit `<script>` blocks whose contents interpolate untrusted strings. Do not render untrusted URLs into `href` or `src` without scheme-checking (`http:` / `https:` / `mailto:` only). This is the HTML-surface equivalent of the formula-injection defence on the Excel side — same threat (attacker-controlled cell content), different execution surface (browser JS instead of spreadsheet formula). A dashboard the reviewer opens in a browser is a trust boundary; treat it like one.
