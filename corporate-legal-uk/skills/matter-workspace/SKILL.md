...
- `/corporate-legal-uk:matter-workspace new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `/corporate-legal-uk:matter-workspace list` — list matters with status and active flag
- `/corporate-legal-uk:matter-workspace switch <slug>` — set the active matter
- `/corporate-legal-uk:matter-workspace close <slug>` — archive a matter (move to `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/matters/_archived/`, never delete)
- `/corporate-legal-uk:matter-workspace none` — detach from any active matter, work at practice-level only

## Instructions

1. Read `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `/corporate-legal-uk:cold-start-interview --redo` and select a private-practice setting. Otherwise, you don't need `/matter-workspace` at all." Don't error — the disabled state is the expected one for in-house users.
2. Use the workflow below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/matters/<slug>/` to `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/matters/_archived/<slug>/`, log the close date in `history.md`.
...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗`, this skill does not run; `/corporate-legal-uk:matter-workspace` explains the disabled state and suggests `/corporate-legal-uk:cold-start-interview --redo` for users who actually need matter isolation.
...
```
~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/
...
Slugs are lowercase with hyphens. Examples: `acme-spa-2026`, `zenith-renewal`, `vendor-xyz-nda`.
...
2. Run the intake interview:
   ...
   - **Matter type** (read the plugin's practice profile for typical categories; for corporate-legal-uk: M&A buy-side | M&A sell-side | financing | board matter | entity reorg | integration project | other)
   ...
   - **Matter-specific overrides to the practice playbook** (e.g., "client requires 24-month LoL cap not 12", "counterparty is a strategic partner — relationship-preserving tone", "governing law: Scots law not English law")
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`/corporate-legal-uk:matter-workspace switch <slug>`)"
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `/corporate-legal-uk:matter-workspace new <slug>`.
...
[M&A buy-side | M&A sell-side | financing | board matter | entity reorg | integration project | other — with one-line rationale]
...
- [e.g., "Governing law: must be Scots law, not English law."]
- [e.g., "Completion mechanism: locked-box (not completion accounts)."]
...
[Any initial context worth preserving beyond matter.md — e.g., "Opened in response to inbound SPA draft from [counterparty]."]
```
...
When it's `on`, a skill may read files across matter folders only when the user explicitly asks it to (e.g., "compare our position on limitation of liability caps across the last five vendor matters"). Even when `on`, the default is to load only the active matter unless the user asks for a cross-matter view.

## What this skill does not do

- **Run a conflicts check.** Conflicts are the practitioner's/firm's job; the intake captures what the user declares. SRA conflict-of-interest rules (Chapter 6, SRA Code of Conduct for Solicitors) require the practice's own processes — this skill does not substitute for them.
