...
- `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` → Deal team briefing (cadence, format, what the business reads)
- `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/deals/[code]/deal-context.md` → deal lead, timeline
- Current findings from diligence-issue-extraction output

## Audience tiers

Per `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` — what the business reads vs. what's for the file. Default tiers:
...
> This brief aggregates privileged diligence findings and inherits the sources' legal professional privilege and confidentiality status. Distribution beyond the privilege circle (including to broader business teams) can waive privilege — confirm the distribution list matches the privilege circle before sending.
...
If this is a recurring brief (per `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` cadence), lead with what changed:
...
- **To closing-checklist:** Any "decision needed" items that resolve into completion conditions go on the checklist.
