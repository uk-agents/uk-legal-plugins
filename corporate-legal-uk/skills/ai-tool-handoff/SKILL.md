...
description: >
  ...
  per the trust level in `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md`. Use when user says "send to Luminance",
...
**Before you hand off:** try `tabular-review` first (`/corporate-legal-uk:tabular-review`). For anything the user's environment can handle — a few hundred documents, a defined column schema — native tabular review is faster to set up, has no per-document cost, and keeps the work product local. Hand off to Luminance/Kira when the corpus is genuinely too large, the team already has a licence and workflow, or the matter requires a tool with a validated provenance chain.

## Load context

`~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` → AI-assisted review:
...
If `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` says this — it's rare.)
...
- Share sale vs. asset sale vs. business transfer vs. merger — different triggers
...
- Scottish-law governed contracts: flag for Scottish counsel review if the Scots law assignment or CoC position is material.

This is the part the tool can't do. Output goes to diligence findings in house format.

## Output

> The QA summary below is derived from VDR documents that are privileged, confidential, or both. It inherits the sources' legal professional privilege and confidentiality status — distribution beyond the privilege circle can waive privilege. Store with the matter's privileged files.
...
**Trust level:** [per `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md`]
...
End with the next-steps decision tree per CLAUDE.md `## Outputs`. Customise the options to what this skill just produced — the five default branches (draft the X, escalate, get more facts, watch and wait, something else) are a starting point, not a lock-in. The tree is the output; the solicitor picks.
...
- It doesn't replace the tool's output with its own judgment entirely — if `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in `~/.claude/plugins/config/uk-legal-plugins/corporate-legal-uk/CLAUDE.md`, set at cold-start based on the team's experience with the tool.
