...
2. If no HRIS is connected and no `~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/leave-register.yaml` exists, prompt
   the attorney to upload a leave spreadsheet or use
   `/employment-legal-uk:log-leave` to add entries.
...
```
/employment-legal-uk:leave-tracker
```

Run this weekly — set a Monday-morning reminder to invoke
`/employment-legal-uk:leave-tracker`. Automated scheduling requires a separate
