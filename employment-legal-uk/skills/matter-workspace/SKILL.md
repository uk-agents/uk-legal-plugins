...
- `/employment-legal-uk:matter-workspace new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `/employment-legal-uk:matter-workspace list` — list matters with status and active flag
- `/employment-legal-uk:matter-workspace switch <slug>` — set the active matter
- `/employment-legal-uk:matter-workspace close <slug>` — archive a matter (move to `~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/matters/_archived/`, never delete)
- `/employment-legal-uk:matter-workspace none` — detach from any active matter, work at practice-level only

## Instructions

1. Read `~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/CLAUDE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `/employment-legal-uk:cold-start-interview --redo` and select a private-practice setting. Otherwise, you don't need `/matter-workspace` at all." Don't error — the disabled state is the expected one for in-house users.
2. Use the subcommand logic below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/matters/<slug>/` to `~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/matters/_archived/<slug>/`, log the close date in `history.md`.
...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗`, this skill does not run; instead it explains the disabled state and suggests `/employment-legal-uk:cold-start-interview --redo` for users who actually need matter isolation.
...
```
~/.claude/plugins/config/uk-legal-plugins/employment-legal-uk/
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`/employment-legal-uk:matter-workspace switch <slug>`)"
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `/employment-legal-uk:matter-workspace new <slug>`.
...
- [e.g., "Governing law: must be English law, not New York law."]
