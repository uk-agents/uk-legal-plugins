...
An outline is a rule library. Wrong rules are worse than missing rules because you revise from them without re-checking. The rule for this skill:
...
- **If the student asks me to fill in a topic without source material:** the default is no — I leave a `[GAP — fill from class notes]` marker and ask Socratic questions to help them fill it from their own notes. The student learns nothing from reading a rule I wrote; they learn from writing it themselves. Only if the student explicitly overrides ("I know, I just want a reference, write it anyway") do I state a majority rule, and every line I'm not fully confident on gets `[UNCERTAIN]` or `[VERIFY]`.
...
**Narrow carve-out — rule contradiction within the student's own materials.** When the student states a rule that **contradicts their own uploaded notes, case brief, casebook excerpt, or earlier outline section**, surface the conflict without filling in the answer. Say:

> "That doesn't match what you wrote at [file / outline section / case brief]. Your earlier note says [exact quote]. Which is right?"

This is not writing for the student — it is pointing the student at two things they already have and asking them to reconcile.

## Load context

`~/.claude/plugins/config/uk-legal-plugins/law-student-uk/CLAUDE.md` → outline preferences (format, depth, existing outlines location).
...
- Module guide (for structure)
- Existing partial outline (extending, not starting fresh)

### Step 2: Structure

Module guide gives the structure. Major topics → subtopics → rules → cases illustrating rules.

If extending: match the existing outline's structure precisely. Don't impose a different organisation.

### Step 3: Build — scaffold first, content from sources

**The scaffold gets built from the module guide and any existing outline.** The scaffold is topics, sub-topics, case slots, exception placeholders — the skeleton without the rules.

**The content gets filled by the student from their notes, casebook, or briefs — or extracted verbatim from source text the student pastes.** If the student has no source for a topic, the skill does not invent; it asks Socratic questions ("What did the lecturer say about *Caparo*?", "Which case illustrates this rule?") and leaves a `[GAP]` marker.

Never skip the scaffold step and just generate a populated outline. That is the failure mode this skill exists to prevent.

Per the student's format. Common UK law school formats:
...
I. [Major topic]
   A. [Subtopic]
      1. Rule: [statement]
         a. [Case name] [OSCOLA cite]: [how it illustrates the rule]
...
```
## [Topic]
- [Rule]. *Case Name* [year] report page.
- Exception: [rule]. *Case Name* [year] report page.
```
...
- `[CHECK CLASS NOTES — lecturer may have emphasised something here]`
- `[EXCEPTION UNCLEAR — casebook mentions an exception, find the rule]`
- `[SCOTS LAW NOTE — check whether this rule diverges in Scots law]` (flag for students studying E&W law who should be aware of the Scots position)

## Citation check

Any case cites, statutory cites, or rule statements I add to the outline from my own knowledge (rather than from source material you pasted) were generated by an AI model and have not been verified. Before you revise from the outline, look up each case on BAILII (www.bailii.org) and each statute on legislation.gov.uk. OSCOLA format: *Case Name* [year] report abbreviation page; Act Name year, s X. AI-generated citations are sometimes fabricated or misquoted, and a wrong rule you memorised is worse than a gap you filled in later.
...
- Guarantee exam coverage. Outline the whole syllabus; the lecturer will test whatever they want.
- **Invent rules to fill gaps.** If I don't have source material and I'm not confident on a rule, the outline gets `[GAP — fill from class notes]` rather than a fabricated rule. Check every `[VERIFY]` and `[UNCERTAIN]` marker before revising from the outline.
