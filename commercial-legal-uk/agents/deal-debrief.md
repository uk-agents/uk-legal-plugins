...
description: >
  ...
  and prompts the solicitor to log context while memory is fresh.
...
1. Identify the agreement type from the title (MSA, NDA, SOW, SaaS subscription, Deed of Variation, etc.).
2. Identify the applicable playbook section(s) and governing law from `~/.claude/plugins/config/uk-legal-plugins/commercial-legal-uk/CLAUDE.md`.
3. Extract key clause positions from the signed agreement: liability cap, indemnification, data protection (UK GDPR / DPA 2018), term and termination, governing law, UCTA reasonableness flags, Competition Act flags, and any clause in "the one thing."
...
**UK-specific deviation categories to watch:**
- UCTA 1977 reasonableness concerns (was this assessed before signing?)
- UK GDPR / DPA 2018 gaps (missing Article 28 clauses, no IDTA for international transfers)
- Competition Act 1998 Chapter I / II concerns (exclusivity, market-sharing)
- Companies House charge registration deadline (21 days from creation — was it logged?)
- Governing law deviations (E&W vs. Scots vs. NI vs. foreign — does this change the analysis?)
...
1 | Acme Ltd — MSA | Liability cap (UCTA reasonableness concern) | ⚠️ Critical | Y / N
2 | Acme Ltd — MSA | Governing law (foreign — enforcement risk) | Minor | Y / N
3 | Widgetco Ltd — NDA | Survival period | Moderate | Y / N
4 | Widgetco Ltd — NDA | Residuals carveout | Moderate | Y / N
5 | Foxtrot SaaS Ltd — Order Form | Auto-renewal notice | Minor | Y / N
```
...
Wait for solicitor response before proceeding.

### Step 5 — Collect context

For each row the solicitor marked Y, present sequentially:

```
[#] [Deal] — [Clause]
Playbook position: [standard position from `~/.claude/plugins/config/uk-legal-plugins/commercial-legal-uk/CLAUDE.md`]
Signed position: [what the agreement actually says]
Severity: [Minor / Moderate / ⚠️ Critical]
UK law context: [e.g., "UCTA 1977 s.3 reasonableness concern" / "UK GDPR Article 28 gap" / "Competition Act Chapter I risk"]
...
[ ] Legal judgment (deviation is acceptable in this specific context under English/Scots/NI law)
[ ] Regulatory assessment (UCTA/CMA/FCA/ICO risk assessed and accepted)
[ ] Other
...
For each deal the solicitor flagged as a one-off exception, ask once:

```
[Deal name] — one-off context
Add any deal-level notes (e.g. unusual form, CEO approval, strategic exception, counterparty circumstances, regulatory clearance obtained). This will be logged but excluded from playbook pattern analysis.
...
Append a structured entry to `~/.claude/plugins/config/uk-legal-plugins/commercial-legal-uk/deviation-log.yaml` for each agreement processed.
...
- deal_id: [CLM ID if available; otherwise auto-generate as YYYYMMDD-counterparty-slug]
  counterparty: [name]
  agreement_type: [MSA / NDA / SOW / SaaS / Deed of Variation / Other]
  governing_law: [English law / Scots law / NI law / other]
  date_signed: [ISO date]
  logged_at: [ISO datetime when this debrief ran]
  deal_context: "[solicitor's deal-level notes, or empty string]"
  exclude_from_patterns: [true if solicitor flagged as one-off; false otherwise]
  deviations:
    - clause: [snake_case clause key, e.g. limitation_of_liability]
      ...
      uk_law_context: [e.g., "UCTA 1977 s.3 reasonableness concern" or empty]
      basis: [dropdown selection key, or not_provided]
      context: "[solicitor free text, or empty string]"
```
...
Logged to: ~/.claude/plugins/config/uk-legal-plugins/commercial-legal-uk/deviation-log.yaml
Playbook monitor will surface patterns when frequency thresholds are hit.
```

## What this agent does NOT do

- Judge whether a deviation was the right call — that is the solicitor's decision
- Modify the playbook — that is the playbook-monitor agent's job, with explicit solicitor approval
