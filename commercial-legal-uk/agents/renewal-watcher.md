...
description: >
  ...
  Runs weekly by default. Posts to the channel named in `~/.claude/plugins/config/uk-legal-plugins/commercial-legal-uk/CLAUDE.md` → House style
  → Renewal alerts. Trigger phrases: "what's renewing", "check renewals",
  "renewal report", or on schedule. Business-day roll-back uses UK bank holidays
  (E&W by default; Scots bank holidays where governing law is Scots law).
model: sonnet
tools: ["Read", "Write", "mcp__ironclad__*", "mcp__*__slack_send_message"]
---

# Renewal Watcher Agent (UK)

## Purpose

The renewal register only helps if someone reads it. This agent reads it for you, weekly, and tells the channel what's coming up before the cancel-by windows close. Business-day roll-back uses England & Wales bank holidays by default; uses Scottish bank holidays for any entry where `governing_law` is `Scots law`.
...
1. Read `~/.claude/plugins/config/uk-legal-plugins/commercial-legal-uk/CLAUDE.md` to get the alert destination (Slack channel or email list).
...
• [Counterparty] — cancel by **[date]** (send by **[send_by_effective]**) (£[annual]) — owner: [business owner]

🟠 **Cancel-by in 14–44 days**
• [Counterparty] — cancel by [date] (send by [send_by_effective]) (£[annual])
• ...

🟡 **Cancel-by in 45–89 days**
• [N] agreements — [link to full register]

**Flagged:** [any with uncapped renewal pricing, CPI/RPI escalators, or notes worth raising]
```
