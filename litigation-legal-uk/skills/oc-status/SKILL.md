...
**Subject:** per house convention (from `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` external solicitors directive style; fallback: `[Matter: [matter name]] — Weekly status update`)

**Body skeleton:**

```
[lead solicitor first name],

[One sentence opener — natural, matches house tone.]

Checking in on [matter name]. A few items:

1. **Status since [date of last update captured in history.md]** — what's moved, what's pending? Any filings, hearings, correspondence with the court or the other side, or calls since we last touched base?
...
3. **Decisions pending** — [pull open questions from matter.md that require external solicitors input; if none, omit this numbered item and renumber]

4. **Budget** — [monthly / quarterly / on-request per `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` budget posture]. Where are we against [budget authorisation from matter.md]? Any variance to flag?

[If material and relevant: 5. Specific ask — e.g., "Please send me the latest draft of the defence before [date]" — drawn from matter.md open questions.]

[Signoff — name, role, contact. From `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` signer default for external solicitors directives.]
```

Adapt tone per `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` external solicitors directive style — some in-house teams are formal; others are first-name-and-bullets. Match.

## Output

### Markdown drafts

Write to: `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/oc-status/[YYYY-MM-DD]/[slug].md`
...
# [Matter name] — External solicitors status request — [YYYY-MM-DD]

**To:** [outside_counsel.email from log] ([outside_counsel.lead], [outside_counsel.firm])
**From:** [signer name / email from `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md`]
**Subject:** [subject line]

> The work-product header above applies to this internal record. The outgoing email body below goes to external solicitors on a retained matter, which is itself a privileged communication — apply the house LPP marking (`~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` privilege conventions) at the top of the email sent, typically `Privileged & Confidential — Legal Professional Privilege`, not this internal work-product header.
...
> This is a draft status email for solicitor review before sending to external solicitors. Check for privileged content you did not intend to share outside the engagement circle, factual accuracy, tone, and budget posture. Do not send unreviewed — even routine weekly check-ins can surface theory, strategy, or concessions the sender did not mean to put in writing.
...
After processing all matters, write `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/oc-status/[YYYY-MM-DD]/_summary.md`:

```markdown
# External Solicitors Status Run — [YYYY-MM-DD]
...
| Matter | Lead solicitor | Last updated | Reason for inclusion |
|---|---|---|---|
...
| [slug] | no solicitor email in log — update with `/matter-update [slug]` |

## Anomalies

- Matters without external solicitors assigned: [list — if any are high/critical risk, flagged]
- Matters with external solicitors but no email in log: [list]
```

## Scheduling

This skill is designed to run weekly. Automated scheduling requires a scheduled-tasks integration that is not bundled with the plugin. To run weekly, set a recurring reminder to invoke `/litigation-legal-uk:oc-status` — e.g., Monday morning on your calendar.
...
- **Rewrite history.md.** Reads it for context; doesn't modify. (If external solicitors' response surfaces new events, use `/matter-update [slug]` to log them.)
- **Enforce a minimum template.** If the house tone is "one line, first name, done," the draft honours that and skips the bulleted structure. Match `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md`.
