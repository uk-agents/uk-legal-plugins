...
description: Roll up the portfolio from _log.yaml — risk distribution, upcoming CPR deadlines, stale matters, materiality totals, stage distribution, and flagged anomalies. Use when the user asks "where do we stand", "how many open matters", or wants a portfolio rollup or status across all active matters.
...
6. **Hold gap:** `status in [threatened, active, disclosure, trial, appeal]` and `legal_hold.issued == false` — preservation duty under English law attaches at reasonable anticipation, so `threatened` matters are in scope.
...
If the portfolio has more than ~10 matters, or any time the user asks: offer the dashboard (see CLAUDE.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by risk tier, a timeline of upcoming CPR deadlines, and a sortable matter ledger with status, conflicts check, and last-touched date.

## What this skill does not do

- Make decisions. It surfaces what needs attention; the user decides priority.
- Pretend precision it doesn't have. Exposure midpoints are rough and should be labelled so.
- Replace a real matter management system. This is a working-memory rollup, not a system of record.
- Calendar CPR deadlines. Deadlines shown in the rollup are from the `next_deadline` field in `_log.yaml`. They are leads requiring verification against the court's case management directions and the applicable CPR rule or pre-action protocol before anyone relies on them.
