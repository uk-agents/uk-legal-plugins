...
2. Confirm slug exists in `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/` and `_log.yaml`.
...
4. Append dated entry to `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/[slug]/history.md`.
...
- `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/[slug]/matter.md` — reference (don't rewrite)
- `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` — risk calibration (if re-assessing risk)
...
> "I don't see [matter slug] in the matter log. Run `/litigation-legal-uk:matter-intake` first so the conflicts check runs and the matter workspace exists. I won't append history to an unmanaged matter — the conflicts check is the gate, and there's no `history.md` to append to until the matter is intaken."
...
- **Procedural** — application filed/received, order issued, hearing held, CPR deadline set or confirmed
- **Disclosure** — disclosure given/received, inspection request, CPR 31.17 third-party disclosure application
...
- **Stakeholder** — new person looped in, external solicitors change, barrister instructed or changed
- **Administrative** — engagement letter executed, budget adjusted, hold refreshed, court fee paid
- **Pre-action** — pre-action protocol steps, letter before action sent/received, protocol compliance completed
...
- `status:` — has the stage shifted (e.g., pre-action → pleadings → disclosure)?
...
- `next_deadline:` — new upcoming CPR or case management date, if any
- `outside_counsel:` — change?
- `internal_owners:` — anyone new or removed?
- `legal_hold:` — refreshed, expanded, released?
- `pre_action_protocol:` — completed or a new protocol identified?
...
If the Strategy update is a **settlement acceptance** (the company is accepting a settlement offer, executing a settlement agreement, or authorising acceptance in principle — not merely logging an offer made or received): Read `## Who's using this` in `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md`. If the Role is Non-lawyer:

> Accepting a settlement has legal consequences — it resolves claims, typically requires a release, and can affect insurance, tax, and related matters. Have you reviewed this with a solicitor or barrister? If yes, proceed. If no, here's a brief to bring to them:
>
> [Generate a 1-page summary: the matter, proposed settlement terms (amount, structural terms, release scope, confidentiality, non-disparagement), exposure at stake, authority ladder status (see `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` settlement authority), what could go wrong, what to ask the solicitor/barrister before accepting.]
>
> If you need to find a licensed solicitor, barrister, or other authorised legal professional: the SRA (solicitors.lawsociety.org.uk) or Bar Standards Board (barstandards board.org.uk/find-a-barrister) for England & Wales; the Law Society of Scotland (lawscot.org.uk) for Scotland; the Law Society of Northern Ireland (lawsoc-ni.org) for Northern Ireland.
...
| Regulatory / enforcement development | "Regulator action (investigation notice, CMA/FCA inquiry, enforcement notice) usually triggers disclosure analysis. Current: `[current]`. Change?" |
...
If materiality moves to `reserved` or `disclosed`, and the matter did not previously carry a reserve or disclosure, flag the event as requiring finance / audit-committee notification per `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` materiality thresholds.

### 5. Seed doc prompt (optional)

If the update references a document (order, court filing, correspondence), ask if there's a path to link. Not pushy.

## Writing

### Append to `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/[slug]/history.md`
...
- Compute CPR deadlines. If the user logs a new procedural event, prompt for the `next_deadline` date but do not attempt to compute it from CPR rules — the applicable CPR Part, Practice Direction, and case management directions determine the deadline, and that's for external solicitors to confirm.
