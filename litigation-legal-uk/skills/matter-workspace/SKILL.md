...
- `/litigation-legal-uk:matter-workspace new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `/litigation-legal-uk:matter-workspace list` — list matters with status and active flag
- `/litigation-legal-uk:matter-workspace switch <slug>` — set the active matter
- `/litigation-legal-uk:matter-workspace close <slug>` — archive a matter (move to `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/_archived/`, never delete)
- `/litigation-legal-uk:matter-workspace none` — detach from any active matter, work at practice-level only

Note: `/litigation-legal-uk:matter-briefing [slug]` (no subcommand) is a separate command that produces a briefing on a specific matter — useful for in-house portfolio review. Matter workspace management lives here.

## Instructions

1. Read `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/CLAUDE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `/litigation-legal-uk:cold-start-interview --redo` and select a private-practice setting. Otherwise, you don't need `/matter-workspace` at all." Don't error — the disabled state is the expected one for in-house users.
2. Follow the workflow and reference below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/<slug>/` to `~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.claude/plugins/config/uk-legal-plugins/litigation-legal-uk/
...
Slugs are lowercase with hyphens. Examples: `acme-contract-2026`, `smith-unfair-dismissal`, `vendor-xyz-ip`.
...
2. Run the intake interview:
   ...
   - **Matter type** (read the plugin's practice profile for typical categories; for litigation-legal-uk: contract dispute | employment | IP | regulatory / investigation | product liability | group litigation | other)
   - **Confidentiality level** (standard | heightened | clean-team — heightened prompts extra care in cross-matter settings)
   - **Key facts** (2–5 sentences: what this matter is about, who the stakeholders are, what is at stake)
   - **Matter-specific overrides to the practice playbook** (e.g., "client requires 24-month limitation of liability cap not 12", "counterparty is a strategic partner — relationship-preserving tone", "Scotland-seated — Scots law applies")
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`/litigation-legal-uk:matter-workspace switch <slug>`)"
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `/litigation-legal-uk:matter-workspace new <slug>`.
2. Edit the `Active matter:` line in the practice-level CLAUDE.md to `Active matter: <slug>`.
3. Show the user the matter.md summary so they can confirm they are on the right matter.
...
[contract dispute | employment | IP | regulatory / investigation | product liability | group litigation | other — with one-line rationale]

## Key facts

[2–5 sentences. What this matter is about. Who the stakeholders are. What is at stake. What makes it different from the default playbook.]

## Matter-specific overrides

*Any deviation from the practice-level playbook that applies to this matter and only this matter.*

- [e.g., "Jurisdiction: Scots law applies — flag for Scottish solicitors on all procedural points."]
- [e.g., "Tone: relationship-preserving — counterparty is a strategic partner."]
- [e.g., "Forum: IPEC — streamlined procedure applies; damages cap £500,000."]
...
[Any initial context worth preserving beyond matter.md — e.g., "Opened in response to inbound Letter Before Action from [counterparty]."]
```

## Cross-matter context

The practice-level CLAUDE.md has a `Cross-matter context:` flag. When it is `off` (the default), a skill working in matter A **never reads** files in `matters/B/` for any other `B`. Period. This is the confidentiality guarantee the setting exists to provide.

When it is `on`, a skill may read files across matter folders only when the user explicitly asks it to (e.g., "compare our position on limitation of liability across the last five vendor matters"). Even when `on`, the default is to load only the active matter unless the user asks for a cross-matter view.

## What this skill does not do

- **Run a conflicts check.** Conflicts are the practitioner's/firm's job; the intake captures what the user declares. For in-house users, the matter-intake skill runs the conflicts check as a gating step.
...
- **Decide whether cross-matter context is appropriate.** It reads the flag and obeys.
