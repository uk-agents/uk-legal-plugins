...
- **Make the decline pathway a scaffold, not an escape hatch.** If the right
  ...
  the skill as a hard gate. `legal-clinic-uk`'s `/deadlines` do-not-compute rule is
...
- **Validate before opening a PR.** Run `claude plugin validate .claude-plugin/marketplace.json` and `claude plugin validate <plugin>/` to check plugin manifests.
